        ...
        with st.form(usr[0]):
            ...
            with sub:
                if st.form_submit_button("通过", use_container_width=True, type="primary"):
                    ...
                    res = xie.change(conn, 2, usr[0], st.session_state.uid)
                    conn.close()
                    if res['status'] == 1:
                        st.rerun()
                    elif res['status'] == 0:
...
def check_page():
    if 'check_page' not in st.session_state:
        st.session_state.check_page = 1
    conn = create_connection()
    search = st.text_input("", placeholder='请输入用户名或uid')
    res = du.find_check_kuaidi(conn, search, st.session_state.check_page)
    conn.close()
    if res['status'] == 1:
        usr_list(res)
        ...
        with up_page:
            if st.button("上一页", use_container_width=True):
                if st.session_state.check_page > 1:
                    st.session_state.check_page -= 1
                    st.rerun()
        with yeshu:
            # 用st.markdown来显示页数，因为st.text无法居中显示页数
            st.markdown("<p style='text-align: center;'>" + str(st.session_state.check_page) + " / "
                        + str(res['total_pages']) + "</p>", unsafe_allow_html=True)
        with down_page:
            if st.button("下一页", use_container_width=True):
                if st.session_state.check_page < res['total_pages']:
                    st.session_state.check_page += 1
                    st.rerun()
    elif res['status'] == 0:
        st.warning(res['message'])
