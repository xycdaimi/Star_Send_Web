...
from send.person import change_my_person, my_person
...
def user():
    if st.session_state.detail != "":
        ...
        # 设置导航栏的宽度
        selected0 = option_menu(None, ["快递", "寄件", '我的'],
                                icons=['envelope', 'send', 'person'],
    ...
    with st.sidebar:
        ...
        change_kuaidi = st.button('成为快递员' if (int(st.session_state.kuaidi) in (0, 3, 4)) else '快递员资格审核中',
                                  use_container_width=True,
                                  disabled=False if (int(st.session_state.kuaidi) == 0) else True)
        change_wangdian = st.button('成为网点管理员' if (int(st.session_state.kuaidi) in (0, 1, 2)) else '网点管理员资格审核中',
                                    use_container_width=True,
                                    disabled=False if (int(st.session_state.kuaidi) == 0) else True)
        qut = st.button('退出登录', key="quit", use_container_width=True)
        if change_wangdian:
            conn = create_connection()
            res = xie.change(conn, 3, st.session_state.uid)
            conn.close()
            if res['status'] == 1:
                st.session_state.kuaidi = 3
                st.rerun()
            elif res['status'] == 0:
        ...
        if change_kuaidi:
            conn = create_connection()
            res = xie.change(conn, 1, st.session_state.uid)
            conn.close()
            if res['status'] == 1:
                st.session_state.kuaidi = 1
                st.rerun()
            elif res['status'] == 0:
                st.warning(res['message'])
            else:
